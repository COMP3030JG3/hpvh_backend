...
def employee_appointment_modify(id):
    ...
    if not DBUtil.modify({'app_primary_key': id}, appointment_res, 'appointment'):
...
def employee_operation_create():
    ...
    appointment, length = DBUtil.search(
        {'app_primary_key': operation_res.get('appointment_id')}, 'appointment')
    if not appointment:
...
# 需要修改customer的profile？
@app.route('/api/employee/modify/<int:id>', methods=['POST'])
@auth.login_required
...
def employee_add():
    ...
    if level == "administrator":
        ...
        # 获得前端来的json数据,格式应该是 {username：,password：,email:, phone_num:, address:, }
        register_res = request.get_json()
        username = {"username": register_res.get(
            "username")}  # 提取有用信息(username)
        if DBUtil.search(username, "employee")[0]:
            return status(4103, 'exist username')
        else:
            return_status = DBUtil.insert(register_res, "employee")
